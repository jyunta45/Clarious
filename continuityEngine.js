...
function storeLoop(userId, message) {
  ...
  if (user.openLoops.length > 10)
    user.openLoops.shift();
}


// --------------------------------------
// RECALL LOOP OCCASIONALLY
// --------------------------------------
function recallLoop(userId) {
  ...
  const randomLoop =
    user.openLoops[
      Math.floor(Math.random() * user.openLoops.length)
    ];
